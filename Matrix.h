...
namespace MatMath {
	...
	Matrix<T>::Matrix(int row, int col)
	{
		...
		memset(m_data, 0, row*col*sizeof(m_data[0]));
	}
...
*/		
	...
	*/
	...
	Matrix<T> Matrix<T>::operator*(const T& c)
	{
		...
		for (unsigned int i{ 0 }; i < m_row*m_col; i++) { result.m_data[i] = m_data[i] * c; }
		return result;
	}
	...
	Matrix<T>& Matrix<T>::operator*=(const T& c)
	{
		for (unsigned int i{ 0 }; i < m_row*m_col; i++) { m_data[i] = m_data[i] * c; }
		return *this;
	}
	...
	Matrix<T>& Matrix<T>::operator+=(const Matrix<T>& M)
	{
		...
		for (unsigned int i{ 0 }; i < m_row*m_col; i++) { m_data[i] = m_data[i] + M.m_data[i]; }
		return *this;
	}
	...
	Matrix<T>& Matrix<T>::operator*=(const Matrix<T>& M)
	{
		...
		for (unsigned int i{ 0 }; i < m_row; i++)
		{
			for (unsigned int j{ 0 }; j < m_col; j++)
			{
				...
				for (unsigned int k{ 0 }; k < m_col; k++)
				{
					m_data[i*m_col + j] += this->m_data[i*m_col + k] * M.m_data[k*M.m_col + j];
				}
			}
		}
		return *this;
	...
	Matrix<T> Matrix<T>::transpose()
	{
		...
		for (unsigned int i{ 0 }; i < m_row; i++)
		{
			for (unsigned int j{ 0 }; j < m_col; j++)
			{
				try { result.m_data[j*m_row + i] = m_data[i*m_col + j].conjugate(); }
				catch (...) { result.m_data[j*m_row + i] = m_data[i*m_col + j]; }
			}
		}
		result.display();
		return result;
